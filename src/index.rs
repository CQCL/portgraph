...
pub enum PortOffset<O: IndexType = u16> {
    /// Input to a node.
    Incoming(O),
    /// Output from a node.
    Outgoing(O),
...
impl<O: IndexType> PortOffset<O> {
    ...
    pub fn new_incoming(offset: usize) -> Self {
        PortOffset::Incoming(O::new(offset))
    }
    ...
    pub fn new_outgoing(offset: usize) -> Self {
        PortOffset::Outgoing(O::new(offset))
    }
    ...
    pub fn index(self) -> usize {
        match self {
            PortOffset::Incoming(offset) => offset.get(),
            PortOffset::Outgoing(offset) => offset.get(),
        }
    }
...
impl<O: IndexType> Serialize for PortOffset<O> {
    ...
    {
        match self {
            PortOffset::Incoming(idx) => {
                serializer.serialize_newtype_variant("PortOffset", 0, "Incoming", &idx.get())
            }
            PortOffset::Outgoing(idx) => {
                serializer.serialize_newtype_variant("PortOffset", 1, "Outgoing", &idx.get())
            }
        }
    }
}
                ...
                A: serde::de::EnumAccess<'de>,
            {
                ...
                match variant {
                    "Incoming" => {
                        ...
                        Ok(PortOffset::Incoming(O::new(idx)))
                    }
                    "Outgoing" => {
                        let idx: usize = value.newtype_variant()?;
                        Ok(PortOffset::Outgoing(O::new(idx)))
                    }
...
impl<O: IndexType> std::fmt::Debug for PortOffset<O> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PortOffset::Incoming(idx) => write!(f, "Incoming({})", idx.get()),
            PortOffset::Outgoing(idx) => write!(f, "Outgoing({})", idx.get()),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_opposite() {
        let incoming = PortOffset::Incoming(5u16);
        let outgoing = PortOffset::Outgoing(5u16);
