...
pub trait PortView {
    /// The index type for nodes in the portgraph.
    type Node: NodeIndex;
    /// The index type for ports in the portgraph.
    type Port: PortIndex;
    /// The index type for port offsets in the portgraph.
    type Offset: PortOffset;

    /// Returns the direction of the `port`.
    #[must_use]
    fn port_direction(&self, port: impl Into<Self::Port>) -> Option<Direction>;

    /// Returns the node that the `port` belongs to.
    #[must_use]
    fn port_node(&self, port: impl Into<Self::Port>) -> Option<Self::Node>;

    /// Returns the index of a `port` within its node's port list.
    #[must_use]
    fn port_offset(&self, port: impl Into<Self::Port>) -> Option<Self::Offset>;

    /// Returns the port index for a given node and offset.
    #[must_use]
    fn port_index(&self, node: Self::Node, offset: Self::Offset) -> Option<Self::Port>;
    ...
    fn ports(
        ...
        node: Self::Node,
        direction: Direction,
    ) -> impl Iterator<Item = Self::Port> + Clone;

    /// Iterates over the input and output ports of the `node` in sequence.
    #[must_use]
    fn all_ports(&self, node: Self::Node) -> impl Iterator<Item = Self::Port> + Clone;
    ...
    #[must_use]
    #[inline]
    fn inputs(&self, node: Self::Node) -> impl Iterator<Item = Self::Port> + Clone {
    ...
    #[must_use]
    #[inline]
    fn outputs(&self, node: Self::Node) -> impl Iterator<Item = Self::Port> + Clone {
    ...
    #[must_use]
    fn input(&self, node: Self::Node, offset: usize) -> Option<Self::Port>;

    /// Returns the output port at the given offset in the `node`.
    ///
    /// Shorthand for [`PortView::port_index`].
    #[must_use]
    fn output(&self, node: Self::Node, offset: usize) -> Option<Self::Port>;
    ...
    #[must_use]
    fn num_ports(&self, node: Self::Node, direction: Direction) -> usize;
    ...
    fn port_offsets(
        ...
        node: Self::Node,
        direction: Direction,
    ) -> impl Iterator<Item = Self::Offset> + Clone;

    /// Iterates over the input and output port offsets of the `node` in sequence.
    #[must_use]
    fn all_port_offsets(&self, node: Self::Node) -> impl Iterator<Item = Self::Offset> + Clone;
    ...
    #[must_use]
    #[inline]
    fn input_offsets(&self, node: Self::Node) -> impl Iterator<Item = Self::Offset> + Clone {
    ...
    #[must_use]
    #[inline]
    fn output_offsets(&self, node: Self::Node) -> impl Iterator<Item = Self::Offset> + Clone {
    ...
    #[must_use]
    fn contains_port(&self, port: Self::Port) -> bool;
    ...
    #[must_use]
    fn nodes_iter(&self) -> impl Iterator<Item = Self::Node> + Clone;

    /// Iterates over the ports in the port graph.
    #[must_use]
    fn ports_iter(&self) -> impl Iterator<Item = Self::Port> + Clone;
    ...
    #[must_use]
    fn node_port_capacity(&self, node: Self::Node) -> usize;
}
...
pub trait PortMut: PortView {
    ...
    /// ```
    fn add_node(&mut self, incoming: usize, outgoing: usize) -> Self::Node;
    ...
    fn set_num_ports<F>(&mut self, node: Self::Node, incoming: usize, outgoing: usize, rekey: F)
    where
        F: FnMut(Self::Port, PortOperation<Self::Port>);

    /// Swaps the indices of two nodes.
    fn swap_nodes(&mut self, a: Self::Node, b: Self::Node);
...
pub trait LinkView: PortView {
    ...
    type LinkEndpoint: Into<Self::Port> + Copy;
    ...
    #[must_use]
    #[inline]
    fn connected(&self, from: Self::Node, to: Self::Node) -> bool {
    ...
    #[must_use]
    #[inline]
    fn port_link(&self, port: Self::Port) -> Option<Self::LinkEndpoint> {
...
pub trait LinkMut: LinkView + PortMut {
    ...
    fn unlink_port(&mut self, port: Self::Port) -> Option<Self::LinkEndpoint>;

    /// Inserts another graph into this graph.
    ///
    /// Returns a map from the old node indices to the new node indices.
    fn insert_graph<Other: LinkView>(
        &mut self,
        other: &Other,
    ) -> Result<HashMap<Other::Node, Self::Node>, LinkError<Self::Node, Self::Port, Self::Offset>>
    {
        ...
        for old in nodes {
            ...
            for (from, to) in other.all_links(old) {
                ...
                let Some(&other_node) = HashMap::get(&rekeys, &other.port_node(to).unwrap()) else {
                    continue;
                };
                self.link_offsets(
                    new,
                    PortOffset::new_outgoing(other.port_offset(from).unwrap().index()),
                    other_node,
                    PortOffset::new_incoming(other.port_offset(to).unwrap().index()),
                )?;
            }
        }
        Ok(rekeys)
...
pub trait MultiView: LinkView {
    ...
    #[must_use]
    fn all_subports(&self, node: Self::Node) -> impl Iterator<Item = Self::LinkEndpoint> + Clone;
    ...
    #[must_use]
    #[inline]
    fn subport_inputs(&self, node: Self::Node) -> impl Iterator<Item = Self::LinkEndpoint> + Clone {
...
pub trait MultiMut: MultiView + LinkMut {
    ...
    ) -> Result<(), LinkError<Self::Node, Self::Port, Self::Offset>>;
