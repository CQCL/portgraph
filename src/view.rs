...
pub trait PortView {
    ...
    type Offset: IndexType;

    /// Returns the direction of the `port`.
    #[must_use]
    fn port_direction(&self, port: impl Into<PortIndex<Self::Port>>) -> Option<Direction>;

    /// Returns the node that the `port` belongs to.
    #[must_use]
    fn port_node(&self, port: impl Into<PortIndex<Self::Port>>) -> Option<NodeIndex<Self::Node>>;
    ...
    fn ports(
        ...
        node: NodeIndex<Self::Node>,
        direction: Direction,
    ) -> impl Iterator<Item = PortIndex<Self::Port>> + Clone;
    ...
    #[must_use]
    fn input(&self, node: NodeIndex<Self::Node>, offset: usize) -> Option<PortIndex<Self::Port>>;

    /// Returns the output port at the given offset in the `node`.
    ///
    /// Shorthand for [`PortView::port_index`].
    #[must_use]
    fn output(&self, node: NodeIndex<Self::Node>, offset: usize) -> Option<PortIndex<Self::Port>>;
    ...
    #[must_use]
    #[inline]
    fn num_inputs(&self, node: NodeIndex<Self::Node>) -> usize {
    ...
    #[must_use]
    #[inline]
    fn num_outputs(&self, node: NodeIndex<Self::Node>) -> usize {
        self.num_ports(node, Direction::Outgoing)
    }

    /// Returns the number of ports of the `node` in the given `direction`.
    #[must_use]
    fn num_ports(&self, node: NodeIndex<Self::Node>, direction: Direction) -> usize;
    ...
    #[must_use]
    fn contains_node(&self, node: NodeIndex<Self::Node>) -> bool;

    /// Returns whether the port graph contains the `port`.
    #[must_use]
    fn contains_port(&self, port: PortIndex<Self::Port>) -> bool;
    ...
    #[must_use]
    fn nodes_iter(&self) -> impl Iterator<Item = NodeIndex<Self::Node>> + Clone;

    /// Iterates over the ports in the port graph.
    #[must_use]
    fn ports_iter(&self) -> impl Iterator<Item = PortIndex<Self::Port>> + Clone;
    ...
    #[must_use]
    fn node_port_capacity(&self, node: NodeIndex<Self::Node>) -> usize;
}
...
pub trait PortMut: PortView {
    ...
    /// ```
    fn add_node(&mut self, incoming: usize, outgoing: usize) -> NodeIndex<Self::Node>;
    ...
    /// ```
    fn remove_node(&mut self, node: NodeIndex<Self::Node>);
    ...
    fn set_num_ports<F>(
        ...
        F: FnMut(PortIndex<Self::Port>, PortOperation<Self::Port>);

    /// Swaps the indices of two nodes.
    fn swap_nodes(&mut self, a: NodeIndex<Self::Node>, b: NodeIndex<Self::Node>);
    ...
    where
        F: FnMut(NodeIndex<Self::Node>, NodeIndex<Self::Node>);
    ...
    where
        F: FnMut(PortIndex<Self::Port>, PortIndex<Self::Port>);
...
pub trait LinkView: PortView {
    ...
    type LinkEndpoint: Into<PortIndex<Self::Port>> + Copy;
    ...
    fn get_connections(
        &self,
        from: NodeIndex<Self::Node>,
        to: NodeIndex<Self::Node>,
    ...
    #[must_use]
    #[inline]
    fn connected(&self, from: NodeIndex<Self::Node>, to: NodeIndex<Self::Node>) -> bool {
    ...
    #[must_use]
    #[inline]
    fn port_link(&self, port: PortIndex<Self::Port>) -> Option<Self::LinkEndpoint> {
    ...
    fn neighbours(
        ...
        node: NodeIndex<Self::Node>,
        direction: Direction,
    ) -> impl Iterator<Item = NodeIndex<Self::Node>> + Clone;
...
pub trait LinkMut: LinkView + PortMut {
    ...
    fn link_ports(
        &mut self,
        port_a: PortIndex<Self::Port>,
        port_b: PortIndex<Self::Port>,
    ) -> Result<
    ...
    fn unlink_port(&mut self, port: PortIndex<Self::Port>) -> Option<Self::LinkEndpoint>;
    ...
    fn insert_graph<Other: LinkView>(
        ...
        HashMap<NodeIndex<Other::Node>, NodeIndex<Self::Node>>,
        LinkError<Self::Node, Self::Port, Self::Offset>,
    > {
